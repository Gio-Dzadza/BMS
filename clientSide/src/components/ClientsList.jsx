...
export default function ClientsList({ updateList, setUpdateList }) {
    ...
    const handleDelete = async(id)=>{
        try {
            ...
            const response = await Axios.delete(`http://localhost:3001/api/clapi/clients/delete/${id}`, {
                headers: {
                    "x-access-token": context.userToken, // Include the token in the headers
                },
            }).then((response)=>{
    ...
    const filteredRows = clientsList && clientsList.filter((client) => {
        ...
        // return (
        //     client.Client_FirstName.toLowerCase().includes(searchByName.toLowerCase()) &&
